...
<?
		...
		if( nLinhas($eventoSorteios)>0 ){
			while( $eventoSorteio = objetoPHP($eventoSorteios) ){
				...
				echo "<h3>".$eventoSorteio->qtde_premio." ".$eventoSorteio->descricao."</h3>";
				...
				if( nLinhas($sorteio)>0 ){
					...
					if( nLinhas($sorteio_regulamentos)>0 ){
						...
						$ganhador = executaSQL("SELECT p.*, es.elemento FROM evento_ganhador eg, participante_cupom pc, elemento_sorteavel es, participante p, participante_unidade pu 
						...
						if( nLinhas($ganhador)>0 ){
							$ganhador = objetoPHP($ganhador);
							echo "<br><br>";
							echo "<strong>Ganhador(a)</strong>: ".$ganhador->nome." - <strong>Número da Sorte</strong>: ".$ganhador->elemento.(($ganhador->elemento!=$elemento)?' (por aproximação)' :'');
						}
