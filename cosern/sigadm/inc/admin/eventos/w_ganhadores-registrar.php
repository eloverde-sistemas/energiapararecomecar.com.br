		...
		if(nLinhas($exe)>0){
			...
			if( $premio->sorteio_nr_extracao>0 ){
				...
				if( $qtdePremio >0 ){
					...
					$elementoSorteExtracao = $premio->sorteio_nr_extracao;

					$elementoMaisProximo = executaSQL("SELECT * FROM elemento_sorteavel es WHERE elemento>='".$elementoSorteExtracao."' AND id_participante_cupom>0 ORDER BY elemento ASC LIMIT 1");
					if( nLinhas($elementoMaisProximo)>0 ){
						$elementoMaisProximo = objetoPHP($elementoMaisProximo);

						echo '<br>Resto: '.$resto = ($elementoSorteExtracao-$elementoMaisProximo->elemento)*-1;

						echo '<br>'.$maisProximo = $elementoMaisProximo->elemento;
					}

					$elementoMaisProximo = executaSQL("SELECT * FROM elemento_sorteavel es WHERE elemento<'".$elementoSorteExtracao."' AND id_participante_cupom>0 ORDER BY elemento DESC LIMIT 1");
					if( nLinhas($elementoMaisProximo)>0 ){
						$elementoMaisProximo = objetoPHP($elementoMaisProximo);

						echo '<br>Resto: '.($elementoSorteExtracao-$elementoMaisProximo->elemento);

						if( ($elementoSorteExtracao-$elementoMaisProximo->elemento) < $resto ){
							echo '<br>'.$maisProximo = $elementoMaisProximo->elemento;
						}
					}
					...
					$ganhadores = executaSQL("SELECT es.id as idElemento, pc.id as idCupom 
												...
												ORDER BY elemento ASC LIMIT $qtdePremio", true);
					...
					if($qtdeGanhadores<$qtdePremio){
						...
						$ganhadores = executaSQL("SELECT es.id as idElemento, pc.id as idCupom FROM elemento_sorteavel es, participante_cupom pc 
													...
													ORDER BY elemento ASC LIMIT $qtdeRestante", true);
						if( nLinhas($ganhadores)>0 ){
							while( $ganhador = objetoPHP($ganhadores) ){
								$qtdeGanhadores++;
								inserirDados("evento_ganhador", array('id_sorteio'=>$premio->id, 'id_participante_cupom'=>$ganhador->idCupom, 'id_elemento'=>$ganhador->idElemento));
