<?php
	...
	$regs = executaSQL("SELECT * FROM participante WHERE REPLACE(REPLACE(cpf, '.', ''), '-', '')=REPLACE(REPLACE('".$cpf."', '.', ''), '-', '') ");
	if( nLinhas($regs)>0 ){
		$reg = objetoPHP($regs);

		$campos = executaSQL("SELECT * FROM evento_participante WHERE id_participante = '".$reg->id."' AND id_evento = '".$_SESSION['campanha']->id."'");
		
		//SE já estiver participando, é para enviar os cupons
		if( nLinhas($campos)>0 ){
