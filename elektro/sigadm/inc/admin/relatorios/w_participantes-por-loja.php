	...
	<div align="center"><h1><?=$translate->translate('participantes_por_loja')?></h1></div>


<?	if($_GET['id']>0){ 

		$lojasPart		= executaSQL("SELECT l.id, l.nome_fantasia, l.cnpj FROM loja l, evento_loja el WHERE el.id_loja=l.id AND el.id_evento='".$_GET['id']."' ORDER BY nome_fantasia");

		$totalPart		= nLinhas(executaSQL("SELECT DISTINCT(id_participante) FROM participante_cupom WHERE id_evento='".$_GET['id']."' AND id_situacao IN (2,90)"));
		
		$totais			= objetoPHP(executaSQL("SELECT COUNT(*) as totalCupons, SUM(valor) as totalConsumo FROM participante_cupom WHERE id_evento='".$_GET['id']."' AND id_situacao IN (2,90)"));
...
<?					
				if(nLinhas($lojasPart)>0){
					...
					while($loja = objetoPHP($lojasPart)){ 
						...
						$cuponsLoja = objetoPHP(executaSQL("SELECT COUNT(*) as totalCupons, SUM(c.valor) as totalConsumo FROM participante_cupom c, loja l WHERE c.id_evento='".$_GET['id']."' AND REPLACE(REPLACE(REPLACE(c.cnpj, '.', ''), '/', ''), '-', '')=REPLACE(REPLACE(REPLACE('".$loja->cnpj."', '.', ''), '/', ''), '-', '') AND c.cnpj =l.cnpj AND c.id_situacao IN (2,90)"));
						
						$partPorLoja = nLinhas(executaSQL("SELECT DISTINCT(id_participante) FROM participante_cupom c, loja l WHERE c.id_evento='".$_GET['id']."' AND REPLACE(REPLACE(REPLACE(c.cnpj, '.', ''), '/', ''), '-', '')=REPLACE(REPLACE(REPLACE('".$loja->cnpj."', '.', ''), '/', ''), '-', '') AND c.cnpj =l.cnpj AND c.id_situacao IN (2,90)"));
...
<?						} ?>
						...
						<tr>
							<td width="240"><span class="neg"><?=$loja->nome_fantasia?></span><br>(<?=$loja->cnpj?>)<?='R:'.($x)?></td>
