		...
		if(nLinhas($exe)>0){
			...
			while( $elemento = objetoPHP($elementos) ){ 
				...
				if( $elemento->id_participante_cupom>0 ){
					$participante = objetoPHP(executaSQL("SELECT p.nome, p.cpf, pc.dt_cadastro FROM participante_cupom pc, participante_unidade pu, participante p
													WHERE pc.id='".$elemento->id_participante_cupom."' 
													AND pc.id_unidade=pu.id AND pu.id_participante=p.id "));

					echo "	|	Participante: ".$participante->nome."-".$participante->cpf."	|	Cupom Registrado em: ".$participante->dt_cadastro;	
				}
			}
